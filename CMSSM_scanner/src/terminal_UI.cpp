...
tuple<double, double, double, double, double, double, double, double, double, double> doPoint(const fs::path& filePath, double& m0_Val, double& mhf_Val) {
    ...
    ******************************************************************/
    ...
    if (!ifs.good()) {
        ...
        return {m0_Val, mhf_Val, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN}; // Adjust according to how you want to handle file read errors
    }
    ...
    if (spinfoValue.find("Point invalid") != string::npos) {
        ...
        return {m0_Val, mhf_Val, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN};
    }
    ...
    double a_t = 0.0, a_c = 0.0, a_u = 0.0, a_b = 0.0, a_s = 0.0, a_d = 0.0, a_tau = 0.0, a_mu = 0.0, a_e = 0.0;
    // Gaugino masses
    ...
    // Soft Higgs masses
    double mHusq, mHdsq;
    mHusq = m0_Val * m0_Val;//getDoubleVecValue("MSOFT", 22);
    mHdsq = m0_Val * m0_Val;//getDoubleVecValue("MSOFT", 21);
    // Soft scalar masses
    double mQ3sq = m0_Val * m0_Val, mQ2sq = m0_Val * m0_Val, mQ1sq = m0_Val * m0_Val;
    ...
    */        
    ...
    double DSN;
    try {
        DSN = DSN_calc(first_GUT_BCs, getmZ2_value, logQSUSY, curr_iter_QGUT);
    } catch (const std::runtime_error& e) {
        std::cerr << "Runtime error: " << e.what() << std::endl;
        DSN = NAN;
    } catch (const std::exception& e) {
    ...
    std::cout << "DEW = " << DEW << "     DBG = " << DBG << "     DHS = " << DHS << "     DSN = " << DSN << endl << "------------------------------------------------" << endl;
    if (isnan(DBG) || isnan(DSN)) {
        return {NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN};
    } else {
        return {m0_Val, mhf_Val, mgl, mh0, mC1, mstop1, DEW, DHS, DBG, DSN};
    }
}

void processFiles(const std::vector<fs::path>& files, const std::vector<double>& m0Values, const std::vector<double>& mhfValues, std::ostream& csvFile) {
    for (const auto& file : files) {
        std::string fileName = file.filename().string();
        std::regex filePattern("spectrum_(\\d+).out");
        std::smatch match;

        if (std::regex_match(fileName, match, filePattern)) {
            int fileNumber = std::stoi(match[1].str());
            int m0Index = (fileNumber - 1) % 32;
            int mhfIndex = (fileNumber - 1) / 32;

            double m0Value = m0Values[m0Index];
            double mhfValue = mhfValues[mhfIndex];

            auto [current_M0, current_MHF, mgl, mh0, mC1, mstop1, DEW, DHS, DBG, DSN] = doPoint(file, m0Value, mhfValue);

            std::lock_guard<std::mutex> guard(csvMutex);
            csvFile << fileNumber << "," << m0Value << "," << mhfValue << "," << mgl << "," << mh0 << "," << mC1 << "," << mstop1 << "," << DEW << "," << DHS << "," << DBG << "," << DSN << "\n";
            csvFile.flush();
        }
    }
}
...
int main() {
    std::vector<double> m0Values(32), mhfValues(32);
    double m0Start = 100.0, m0End = 9000.0, mhfStart = 100.0, mhfEnd = 2000.0;
    for (int i = 0; i < 32; ++i) {
        m0Values[i] = m0Start + i * ((m0End - m0Start) / 31.0);
        mhfValues[i] = mhfStart + i * ((mhfEnd - mhfStart) / 31.0);
    }

    fs::path dirPath("/home/dakotah/softsusy/softsusy-4.1.13/CMSSM_tests/");
    std::ofstream csvFile("/mnt/c/Users/dakot/Documents/Research/DSN4SLHA/CMSSM_m0_vs_mhf_scan/CMSSMscanResults.csv");
    csvFile << "FileNo,m0,mhf,mgl,mh0,mC1,mstop1,DEW,DHS,DBG,DSN\n";
    ...
    std::vector<std::vector<fs::path>> threadFiles(maxThreads);

    // Distribute files among threads
    for (size_t i = 0; i < filePaths.size(); ++i) {
        threadFiles[i % maxThreads].push_back(filePaths[i]);
    }

    // Start threads
    for (int i = 0; i < maxThreads; ++i) {
        threads.emplace_back(processFiles, std::ref(threadFiles[i]), std::ref(m0Values), std::ref(mhfValues), std::ref(csvFile));
    }
